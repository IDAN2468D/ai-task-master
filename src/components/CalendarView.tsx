            ...
            <div className="vibrant-card p-6 md:p-8 bg-white/80 dark:bg-[#111C44]/80 backdrop-blur-xl border border-slate-200 dark:border-white/10 shadow-2xl">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                    <div className="flex items-center gap-5">
                        <div className="w-14 h-14 bg-gradient-stat-1 rounded-2xl flex items-center justify-center shadow-lg shadow-[#4318FF]/20">
                            <CalIcon className="w-7 h-7 text-white" />
                        </div>
                        <div>
                            <h3 className="text-3xl font-black text-slate-800 dark:text-white">
                                {MONTHS_HE[month]} <span className="text-[#4318FF]">{year}</span>
                            </h3>
                    ...
                    <div className="flex items-center gap-3 bg-slate-100/50 dark:bg-black/20 p-2 rounded-2xl border border-slate-200 dark:border-white/5">
                        <button onClick={goToday} className="px-5 py-2.5 bg-white dark:bg-[#0B1437] text-slate-700 dark:text-white rounded-xl text-xs font-black shadow-sm hover:scale-105 transition-transform">היום</button>
                        <div className="h-6 w-px bg-slate-200 dark:bg-white/10 mx-1" />
                        <button onClick={prevMonth} className="p-2.5 rounded-xl text-slate-500 hover:text-[#4318FF] hover:bg-white dark:hover:bg-[#0B1437] transition-all">
                            <ChevronRight className="w-5 h-5" />
                        </button>
                        <button onClick={nextMonth} className="p-2.5 rounded-xl text-slate-500 hover:text-[#4318FF] hover:bg-white dark:hover:bg-[#0B1437] transition-all">
                ...
                <div className="grid grid-cols-7 gap-2 mb-4">
                    {DAYS_HE.map(day => (
                        <div key={day} className="text-center text-[10px] font-black uppercase tracking-widest text-[#4318FF] py-3 bg-[#4318FF]/5 rounded-xl">
                            {day}
                        </div>
                    ))}
                </div>
                    ...
                    {calendarDays.map((day, i) => {
                        ...
                        return (
                            <motion.div
                                ...
                                className={`group min-h-[120px] p-2 rounded-2xl border transition-all cursor-pointer relative overflow-hidden ${!day.isCurrentMonth
                                    ? 'bg-slate-50/30 dark:bg-transparent border-transparent opacity-20 pointer-events-none'
                                    : _isToday
                                        ? 'bg-gradient-to-br from-[#4318FF]/10 to-transparent border-[#4318FF]/40 shadow-xl shadow-[#4318FF]/10'
                                        : 'bg-white/50 dark:bg-[#111C44]/30 border-slate-100 dark:border-white/5 hover:border-[#4318FF]/40 hover:bg-white dark:hover:bg-[#111C44]/50'
                                    }`}
                            >
                                <div className="flex justify-between items-start mb-2">
                                    <span className={`text-xs font-black ${_isToday ? 'text-white bg-[#4318FF] w-7 h-7 rounded-lg flex items-center justify-center shadow-lg' :
                                        'text-slate-700 dark:text-slate-300'
                                        }`}>
                                        {day.date}
                                    </span>
                                    {isBusy && day.isCurrentMonth && (
                                        <div className="flex items-center gap-1 text-[8px] font-black text-amber-500 bg-amber-500/10 px-1.5 py-0.5 rounded-md">
                                ...
                                <div className="space-y-1 relative z-10">
                                    {dayTasks.slice(0, 3).map(task => (
                                        <motion.div
                                            ...
                                            className="flex items-center gap-1.5 px-2 py-1.5 rounded-xl bg-slate-100 dark:bg-[#0B1437]/50 border border-transparent hover:border-[#4318FF]/30 transition-all group/task"
                                        >
                                    ...
                                    {dayTasks.length > 3 && (
                                        <div className="text-[9px] font-black text-[#4318FF] dark:text-[#00E5FF] pt-1 px-1 flex items-center gap-1">
                                ...
                                {/* Hover Invite */}
                                {day.isCurrentMonth && (
                                    <div className="absolute inset-0 bg-[#4318FF]/5 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                        <div className="bg-[#4318FF] text-white p-2 rounded-xl scale-75 group-hover:scale-100 transition-transform shadow-xl">
